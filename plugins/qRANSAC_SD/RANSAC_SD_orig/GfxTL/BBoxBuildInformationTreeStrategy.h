	...
	struct BBoxBuildInformationTreeStrategy
	{
		...
		protected:
			...
			void InitRootBuildInformation(BuildInformationT *bi)
			{
				...
				BaseType::AssignAsAABoxMin(this->at(Dereference(i)), &bi->m_bbox[0]);
				BaseType::AssignAsAABoxMax(this->at(Dereference(i)), &bi->m_bbox[1]);
				for(++i; i != bi->Range().second; ++i)
					BaseType::IncludeInAABox(this->at(Dereference(i)), bi->BBox());
			}
